...
public class TCPClient {
    ...
    public void tryLogin(String username) {
        ...
        // Hint: Reuse sendCommand() method
        String command = "Enter username " + username;
    ...
    private String waitServerResponse() {
        ...
        try {
            receivedMessage = this.fromServer.readLine();
        } catch (IOException e) {
            e.printStackTrace();
            System.err.println(e.getMessage());
            this.disconnect();
    ...
    private void parseIncomingCommands() {
        while (isConnectionActive()) {
            ...
            switch (commandPart) {
                ...
                case "loginerr":
                    onLoginResult(false, responsePart);
                    break;

                // TODO Step 5: update this method, handle user-list response from the server
                // Hint: In Step 5 reuse onUserList() method
                case "users":
                    onUsersList(responsePart.split(" "));
                    break;

                case "msg":
                    onMsgReceived(false, commandPart, responsePart);
                    break;

                case "privmsg":
                    onMsgReceived(true,commandPart,responsePart);
                    break;

                case "msgerr":
                    onMsgError(responsePart);
                    break;

                case "cmderr":
                    onCmdError(responsePart);
                    break;

                case "supported":
                    onSupported(responsePart.split(" "));
                    break;
            }
